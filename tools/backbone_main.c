...
static const struct {
	...
	{ CMD_SHOW, backbone_show,
	  "show information about labels and nexthops" },
...
static void backbone_usage(FILE *file)
{
	...
	fprintf(file,
		"  " BPF_BACKBONE_MAP_ENV
		": bpf map path (relative or absolute) for backbond nexthops, defaults to " BPF_BACKBONE_MAP_DEF_PATH
		"\n");
	fprintf(file,
	...
	for (size_t i = 0; i < sizeof(subcommands) / sizeof(subcommands[0]);
		...
		if (!strcmp(argv[0], subcommands[i].subcommand)) {
			...
			if (!nexthop_map || !strlen(nexthop_map)) {
				snprintf(BPF_BACKBONE_MAP_PATH, PATH_MAX,
					 "%s/%s", BPF_MNT,
					 BPF_BACKBONE_MAP_DEF_PATH);
			} else if (nexthop_map[0] == '/') {
				strncpy(BPF_BACKBONE_MAP_PATH, nexthop_map,
					PATH_MAX);
			} else {
				snprintf(BPF_BACKBONE_MAP_PATH, PATH_MAX,
					 "%s/%s", BPF_MNT, nexthop_map);
			}
