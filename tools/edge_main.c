...
static void edge_usage(FILE *file)
{
	...
	fprintf(file,
		"  " BPF_EDGE_MAP_ENV
		": bpf map path (relative or absolute) for edge labels, defaults to " BPF_EDGE_MAP_DEF_PATH
		"\n");
	fprintf(file,
	...
	for (size_t i = 0; i < sizeof(subcommands) / sizeof(subcommands[0]);
		...
		if (!strcmp(argv[0], subcommands[i].subcommand)) {
			...
			if (!label_map || !strlen(label_map)) {
				snprintf(BPF_EDGE_MAP_PATH, PATH_MAX, "%s/%s",
					 BPF_MNT, BPF_EDGE_MAP_DEF_PATH);
			} else if (label_map[0] == '/') {
